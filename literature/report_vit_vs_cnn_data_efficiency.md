...
The results cut both ways. Pre-trained on JFT-300M (300 million images), the largest ViT variants achieved 88.55% top-1 accuracy on ImageNet, surpassing the best CNNs. But with only ImageNet-1K pre-training, ViT-Large/16 managed only 76.53% — roughly ResNet-50 territory, despite having far more parameters [@dosovitskiy2021vit]. The paper states this directly: transformers "do not generalize well when trained on insufficient amounts of data."

The paper also varied pre-training dataset size directly. With ImageNet-1K pre-training, BiT ResNets outperformed ViTs, and larger ViT models degraded more than smaller ones (ViT-L underperformed ViT-B). At the scale of ImageNet-21K (14M images), ViT and CNN performance converged, and only at JFT-300M did ViT pull ahead decisively. The pattern suggests a performance crossover between architectures that occurs somewhere in the tens of millions of images.
...
First, ViTs have a much more **uniform representation structure** across layers. In a ResNet, lower and higher layers have clearly different representations, with distinct transitions between stages. In a ViT, representations are more similar across the full depth of the network — the lower layers already contain a mix of local and global information.
...
Third, the effect is worse for larger models. ViT-B/32 (the smallest variant tested) still managed to learn local attention when trained on ImageNet alone and showed "similar performance in both settings" (JFT-pretrained vs. ImageNet-only). But ViT-L/16 and ViT-H/14 showed clear degradation, suggesting that larger models require more data to converge to useful representations.

The dataset scale effects were not uniform across the network. Even with only 3% of the training data, lower-layer representations closely resembled those learned from the full dataset. It was the higher layers that required larger amounts of data to converge [@raghu2021dovit].
...
**DeiT** [@touvron2021deit] demonstrated that ViTs can be trained competitively on ImageNet-1K alone, without any external data, given the right training recipe. The key ingredients were aggressive data augmentation (RandAugment, Mixup, CutMix, random erasing), careful regularization (stochastic depth, repeated augmentation), and a long training schedule (300 epochs). DeiT-Base achieved 81.8% top-1 on ImageNet-1K, and with a higher resolution fine-tuning step, DeiT-Base reached 83.1% — approaching EfficientNet-B4, though at lower throughput.

DeiT also introduced a **distillation token** that enables knowledge transfer from a CNN teacher to a ViT student. Interestingly, CNN teachers produced better ViT students than transformer teachers of comparable accuracy [@touvron2021deit]. The authors attributed this probably to the transfer of inductive biases through the distillation process. Hard distillation (using the teacher's argmax labels) significantly outperformed soft distillation (83.0% vs. 81.8% for DeiT-B at 224x224), with DeiT reaching 85.2% when combining distillation with fine-tuning at 384x384 resolution.

The smaller dataset experiments in DeiT are worth noting for our project. DeiT-B trained from scratch on CIFAR-10 (50K images) reached 98.0% accuracy, compared to 99.1% with ImageNet pre-training and fine-tuning. The gap between scratch and pre-trained is meaningful but not catastrophic on CIFAR-10 — though the gap would likely widen on more complex tasks with similar data volumes.

**Steiner et al.** [@steiner2022trainvit] ran a large-scale study of training recipe effects, using the shorthand "AugReg" (augmentation + regularization). Their central finding: **the right combination of augmentation and regularization can substitute for approximately 10x more training data**. ViT models of various sizes trained on ImageNet-1K with carefully tuned AugReg matched the performance of the same models trained on ImageNet-21K (10x more data) without AugReg.

Some practical takeaways from this work. First, augmentation tends to help more than regularization; when compute is limited, invest in augmentation first [@steiner2022trainvit]. Second, more data leads to more generic models that transfer better, but AugReg partially closes this gap. Third, for small downstream datasets (e.g., Oxford Pets with ~3K images or RESISC45 with ~30K images), transfer learning from a pre-trained model always outperformed training from scratch, regardless of how good the AugReg recipe was. Fourth, when choosing model variants under a fixed compute budget, prefer a larger model with bigger patches (e.g., ViT-B/32) over a thinner model with smaller patches (e.g., ViT-Ti/16) — the reduced spatial resolution hurts less than the loss of model capacity.
...
**CoAtNet** [@dai2021coatnet] systematically studied how to stack convolution and attention layers for different data regimes. The paper articulated the core tradeoff: convolutional layers tend to have better generalization thanks to their strong inductive bias, while attention layers have higher model capacity that can benefit from larger datasets. Dai et al. evaluated every possible combination of convolution (C) and transformer (T) stages in a four-stage architecture and found:
...
The pattern is clear: **the benefit of convolutional inductive bias is inversely proportional to dataset size** [@dascoli2021convit]. At 5% of ImageNet (about 64K images — comparable to PASCAL VOC scale), the soft convolutional bias provides a 37% relative improvement. At 100% of ImageNet, the benefit shrinks to 2%. d'Ascoli et al. also found that the benefit of inductive bias increases with model size — larger models benefit more from the convolutional prior, particularly on small datasets.

One experiment from ConViT really drives this home: **freezing** the GPSA layers entirely with convolutional initialization (so they act as fixed convolutions, with only the FFN layers learning) produced a model that still outperformed DeiT on 10% of ImageNet data (54.3% vs. 47.8%). With random initialization instead, the frozen model scored only 44.8% — still close to DeiT, but the convolutional initialization clearly mattered. Even a fixed convolutional prior was better than no prior at all in the low-data regime.
...
Replacing this patchify stem with a small convolutional stem of ~5 3x3 convolutions with a mix of stride-2 and stride-1 layers (dubbed ViT_C) improved nearly everything: 1-2% accuracy gains on ImageNet-1K, faster convergence, robustness to optimizer choice (the accuracy gap between AdamW and SGD shrank from ~10% to less than 0.2% for larger models), and better hyperparameter stability [@xiao2021earlyconv].

Here's the uncomfortable part: on ImageNet-1K, the baseline ViT with patchify stem (ViT_P) **underperformed both RegNetY and ResNet** across the entire model complexity spectrum [@xiao2021earlyconv]. ViT_P didn't outperform state-of-the-art CNNs even with ImageNet-21K pre-training. ViT_C was more competitive, outperforming CNNs in the middle-complexity range on ImageNet-1K alone and strictly surpassing both ViT_P and RegNetY with ImageNet-21K pre-training. The benefit of the convolutional stem was consistent across all data scales, suggesting it is not merely a small-data crutch but a structural improvement to ViT's optimization landscape.
...
- In most reviewed studies, ViT-based models or ViT-CNN hybrids matched or outperformed standalone CNNs, though results in medical imaging were mixed — some studies showed ViT superiority while others found ViTs failed to generalize with small medical datasets.
...
- **Pre-training was identified as important** in several individual studies. One study explicitly found that pre-trained CNNs beat ViTs, but ViTs beat non-pre-trained CNNs — suggesting that training regime can matter more than architecture choice.
- **Hybrid models (combining CNN and ViT components) outperformed both standalone architectures** in the few reviewed studies that tested them.

The survey's bottom line: "CNN's can generalize better with smaller datasets and get better accuracy than ViTs, but in contrast, ViTs have the advantage of learning information better with fewer images" [@mauricio2023survey]. One way to read this: CNNs may have stronger test-time generalization on unseen data, while ViTs may extract richer information from individual examples. That tension is worth keeping in mind as we design our experiments.
...
**Swin Transformer** [@liu2021swin] showed that transformers could borrow CNN design principles (hierarchical feature maps, local windowed attention with linear complexity) and compete on ImageNet-1K without needing JFT-scale data. Swin-T achieved 81.3% on ImageNet-1K, surpassing both DeiT-S (79.8%) and ResNet-50. With ImageNet-22K pre-training, Swin-L reached 87.3%. Swin was among the first transformers to work well as a general-purpose vision backbone across classification, detection, and segmentation at practical data scales.
